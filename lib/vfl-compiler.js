    ...
    parse: function(input, startRule) {
      ...
      function computeErrorPosition() {
        ...
        parser.addChains = function (views,o) {
          var chains, chain, prop, preds, connector, ccss;    
          chains = o.chains
          if (chains) {            
            for (var i = 0; i < chains.length; i++) {
              ...
              prop = chain[0]
              preds = chain[1]  
              if (preds === "") {preds = [parser.defaultChainObject];} // load default chain predicate
              for (var j = 0; j < preds.length; j++) {
                pred = preds[j];
                ccss = ""
                for (var k = 0; k < views.length - 1; k++) {
                  view = views[k];  
                  if (pred.headEq === "") {pred.headEq = parser.defaultChainObject.headEq}
                  ccss += " " + view + "[" + prop + "] " + pred.headEq
                  if (pred.value !== "") {
                    ccss += " " + pred.value              
                    if (views.length > 1) {
                      if (pred.tailEq === "") {pred.tailEq = parser.chainTailEqMap[pred.headEq]}
                      ccss += " " + pred.tailEq
                    }
                    else {
                      ccss += " " + pred.s
